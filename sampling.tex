...
% prettier multiple authors (2013-03-12, JM)
\usepackage{authblk}

\title{Social Media Activity \\ \large{Time Series Signals and Sampling} }
\author[]{Scott Hendrickson}

% If we want to include these at some point:
\author[]{Brian Lehman}
...
%\author{Scott Hendrickson, Brian Lehman, Joshua Montague \newline Gnip, Inc.}

\maketitle

\section{Introduction}

For many topics of interest, social media streams contain data with significant volume and frequency to create reliable, high-resolution signals in the time series.  But for the long tail of topics, we need to take some care in identifying the time scale over which an activity time series signal is meaningful. This white paper addresses questions related to social media activity time series sampling, signal, and confidence. We start with short discussion of Gnip's Twitter sampling algorithm, and they address the process of choosing sample sizes, time periods and signal strengths for reliable signals.

\subsection{Sampling and Filtering} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

There are two approaches to sampling the a firehose of social data. Both involve reducing the number of activities in the stream to an intermediate and manageable size for further analysis.  First, Gnip provides a filtering on keywords to select only the portion of the stream that is relevant to the topic you want to analyze. For example, if you are interesting in tracking the super bowl, you might start with a broad stream defined by the terms ``superbowl'' ``super bowl'' and ``contains:xxiii'', the latter for the number of the Superbowl that might be seen in hashtags or short links. This would limit the social media stream to only the activities explicitly related to the Superbowl.

A second approach would be to filter this stream to known fraction of the total firehose. For example, the Superbowl stream of activities might be very large on Twitter, so we might add a sampling operator to talk only 12\% of the matched tweets. For example, we can add ``sample:12''.
...
\begin{enumerate}
	\item 1\% resolution
	\item Progressively inclusive. E.g. the 2\% stream includes the activities from the 1\% stream, plus another 1\%, and so on \ldots independent of keyword filter.
	\item Sample tweets are identified as portion of full stream belonging to sample, then filtered by keywords.
\end{enumerate}

From these three properties, it is clear that in the long term, the sample of 12\% of the stream filtered on activities containing ``superbowl'' ``super bowl'' and ``contains:xxiii'' (let's say that is 20\% of the stream to make this a concrete example).  In this case we expect to observe:

\begin{equation}
    \label{eq:sbsample}
    N_{obs,superbowl} = N_{activities} \times sample \times matching = 0.10(0.20)N_{activities}
\end{equation}

It is fairly clear that if Gnip were to filter on keywords then follow this with sampling, that this would be a reasonable estimate of $N_{obs,superbowl}$ on time scale of the sampling calculation. However, this process would require relaxing prperty 2 in the list above.  Do the sample followed by the keyword filter gives a slightly more complicated short term behavior.

The core question becomes: ``For a required level of certainty that a signal change has been observed, how long or how many events must we observe?''

The following addresses this question in general.  The last section illustrates the general discussion with specific examples.
...
If you have been asked questions like these, you may be interested to understand this paper and the it may be useful for you to review the sample calculations to understand how to estimate the trade-off of sampling time, activity rate and signal strength.

\begin{itemize}
\item I plan to bucket data to estimate rate, how long (time) should the buckets be? 
\item How many activities should I target to collect per bucket?
\item The activity rate has doubled from 5 counts, to 10 counts between two of my buckets. Is this a signal of change? Or merely expected variation for low-frequency events?
