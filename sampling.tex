...
rate. Such changes may be signals of emerging events or conversations and we would like to 
quantify our ability to identify these kinds of signals. Figure~\ref{fig:tradeoff} illustrates 
(schematically) the three main parameters involved in such calculations: signal size, total 
...
activities is a function of the observation time and underlying instantaneous rate, and the 
...
by which a user might collect data from Gnip, specifically. In Sections~\ref{sig} and \ref{time}, we 
outline the mathematical framework for calculations of activity rate and sampling statistics. Finally, 
in Section~\ref{examples}, we work through some example calculations.
...
Examples of typical questions regarding activity rate, signal and confidence level:
...
buckets. Is this change significant, or is this expected variation e.g. due to low-frequency events?
	\item I want to minimize the total number activities that I consume. How can I use a sampling factor 
and still detect a change of 2x in activity rate in 1 hour?
	...
	\item How do I define confidence levels on rate estimates for an activity time series with only twenty events per day?
...
This section is clearly about PowerTrack and then the Sampling operator.
