...
We often wish to calculate social activity rates to identify signals of emerging topics or stories. We want to know what we can detect and with what certainty. To do so, we need to calculate the relationship between activity counts, signal sensitivity and confidence. This relationship is illustrated in Figure~\ref{fig:tradeoff}. The number of activities per time defines the event rate. The signal is the change in the event rate. The confidence around the event rate comes from the statistics of the activities and sampling operators. Time enters the calculation because the longer we count, the more activities we count.
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
\caption{Signal calculation relationships. }
\label{fig:tradeoff}
\end{figure}
%
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Summary of Parameters and Trade )ffs}
